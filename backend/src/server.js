...
app.get("/api/public/framing-offers/:token",async(request,response,next)=>{try{const item=(await query(`SELECT r.*,(SELECT id FROM product_images WHERE product_id=r.product_id AND image_type='primary' ORDER BY sort_order,id LIMIT 1) AS image_id FROM framing_requests r WHERE r.offer_token=$1 AND r.offer_sent_at IS NOT NULL`,[String(request.params.token||"")])).rows[0];if(!item)return response.status(404).json({error:"Offer not found"});response.set("Cache-Control","no-store");response.json({offer:{id:item.id,token:item.offer_token,locale:item.locale==="es"?"es":"de",productTitle:item.product_title,artistName:item.artist_name,format:item.format,formatLabel:({A4:"20 × 30 cm",A3:"30 × 40 cm",A2:"40 × 60 cm"})[item.format]||item.format,image:item.image_id?`/api/public/images/${item.image_id}`:null,description:item.quote_description||"",priceCents:Number(item.quote_price_cents||0),customerName:item.customer_name,customerEmail:item.customer_email,status:item.status,paid:Boolean(item.paid_at||item.status==="paid")}});}catch(error){next(error);}});
