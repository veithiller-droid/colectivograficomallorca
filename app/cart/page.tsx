...
export default function CartPage() {
  ...
  const checkout = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/public/checkout`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ locale: language, items: items.map(item => ({ type: item.type, productId: item.productId, format: item.format, frameId: item.frameId, quantity: item.quantity })) }) });
      const data = await response.json();
      if (!response.ok || !data.url) throw new Error(data.error || "Checkout unavailable");
      window.location.assign(data.url);
    } catch { setError(de ? "Checkout konnte nicht gestartet werden." : "No se pudo iniciar el pago."); setLoading(false); }
  };
