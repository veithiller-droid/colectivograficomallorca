...
export default function ShopPage() {
  return <main><SiteHeader /><section className="shop-intro shell"><p className="eyebrow">Colección</p><h1>Obra gráfica</h1><p>Ediciones creadas en Mallorca. Seleccionadas y producidas en Artà.</p></section>
    ...
    <section className="catalog shell" aria-label="Diseños disponibles">{catalog.map(item => <article className="catalog-card" key={item.id}><div className={`catalog-poster ${item.tone}`}><span>{String(item.id).padStart(2, "0")}</span><div className="mini-shape"/><b>COLECTIVO<br/>GRÁFICO<br/>MALLORCA</b></div><div className="catalog-meta"><div><h2>{item.title}</h2><p>{item.artist}</p></div><span>Próximamente</span></div></article>)}</section>
  </main>;
}
