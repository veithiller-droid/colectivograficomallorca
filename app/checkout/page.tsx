...
export default function CheckoutPage() {
  ...
  return <main><SiteHeader /><section className="checkout-page shell"><div className="checkout-heading"><p className="eyebrow">COLECTIVO GRÁFICO MALLORCA</p><h1>{de ? "Kasse" : "Pago"}</h1><Link href="/cart">← {de ? "Warenkorb bearbeiten" : "Editar cesta"}</Link></div>
    {!items.length ? <div className="empty-cart"><p>{de ? "Dein Warenkorb ist leer." : "Tu cesta está vacía."}</p><Link className="text-link" href="/shop">{de ? "Zum Shop" : "Ir a la tienda"}</Link></div> : <div className="checkout-layout"><div>{error && <p className="checkout-error">{error}</p>}{!clientSecret && !error && <p>{de ? "Zahlung wird vorbereitet …" : "Preparando el pago…"}</p>}{clientSecret && <Elements stripe={stripePromise} options={{ clientSecret, appearance, locale: language === "es" ? "es" : "de" }}><CheckoutForm /></Elements>}</div>
      <aside className="checkout-order"><p className="eyebrow">{de ? "Deine Bestellung" : "Tu pedido"}</p>{items.map(item => <div className="checkout-order-item" key={item.key}>{item.image ? <img src={item.image} alt="" /> : <div className="surprise-mark">5×</div>}<div><strong>{item.title}</strong><small>{item.type === "product" ? `${item.format} · ${item.quantity}×` : `${item.quantity}×`}</small></div><span>{money(item.unitPrice * item.quantity)}</span></div>)}<div className="cart-total"><span>{de ? "Gesamtsumme" : "Total"}</span><strong>{money(total)}</strong></div></aside></div>}
  </section></main>;
}
