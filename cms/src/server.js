...
app.post("/api/framing-requests/:id/send-offer", requireLogin, async (request,response) => {
  const result = await backend(`/api/cms/framing-requests/${encodeURIComponent(request.params.id)}/send-offer`, {method:"POST",body:JSON.stringify({})});
  response.status(result.status).send(await result.text());
});
